...
describe('actions', () => {
  ...
  describe('QUEUE', () => {
    ...
    it('adds the item to items', () => {
      ...
      expect(items).toContainEqual({ ...item, quantity: payload.quantity });
    });
  });
  ...
  describe('REFRESH_SETTLED', () => {
    ...
    it('resets refreshing flag', () => {
      const { isRefreshing } = setup({ state, payload });
      expect(isRefreshing).toBe(false);
    });

    it('removes existing error message', () => {
      const { error } = setup({ state, payload });
  ...
  describe('UPDATE_SETTLED', () => {
    ...
    const cartId = 'new-cart-id';
    const payload = { item: { ...item, cartId } };

    it('updates the item', () => {
      const { items } = setup({ state, payload });

      expect(items[0].cartId).toEqual(cartId);
    });
  ...
  describe('UPDATE_ERROR', () => {
    ...
    const payload = { item, error: 'this new error message' };

    it('empties the queue', () => {
      const { queue } = setup({ state, payload });
  ...
  describe('default', () => {
    const unlisted = action('not listed');

    it('throws an error when not choosing a proper type', () => {
      expect(() => unlisted()).toThrow();
    });
  });
});
