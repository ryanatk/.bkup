...
const Shopping = ({ location }) => {
  ...
  const { eventId, ooSummaryId, scrollPosition, ...shop } = useShop();
  ...
  useEffect(() => {
    ...
    const resetScrollPosition = () => updateShop({ scrollPosition: 0 });
    console.log({ scrollPosition, isCatalog });

    if (scrollPosition && isCatalog) {
      console.log('wtf');
  ...
  }, [location, scrollPosition, updateShop]);
