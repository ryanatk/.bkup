...
describe('Header Component (Horizon)', () => {
  ...
  test.skip(`Renders the CTA with default label when none is provided`, () => {
    render(<Header />);
    expect(screen.getAllByText('header_shop_now')[0]).toBeTruthy();
  });

  test.skip(`Renders the CTA with custom label when one is provided`, () => {
    const label = 'Test Shop Now';
    render(<Header shopButton={{ label: label }} />);
    expect(screen.getAllByText(label)[0]).toBeTruthy();
  });

  test.skip(`Doesn't render the discount message by default`, () => {
    render(<Header />);
    expect(screen.queryByTestId('discount-message')).toBeNull();
  });

  test.skip(`Renders the discount message when showDiscountBanner is true`, () => {
