...
const StartOrder = () => {
  ...
  const { entry, error, isLoading } = useEnterOrder({ eventId, ooSummaryId });
  ...
  return (
    <Content
      error={error?.message || initError?.message || ERROR_MESSAGE}
      isLoading={isLoading || isLoadingInit || isLoadingOrders}
    />
  );
};
