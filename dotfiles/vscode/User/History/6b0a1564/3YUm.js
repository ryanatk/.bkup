import toggleElementAttr, {
  getDataAttr,
  PRECEDENCE_LABEL,
} from './toggleElementAttr';

const ATTR_NAME = 'attr-name'; // i.e. `tabindex` or `aria-hidden`
const TOGGLED_VALUE = 'toggled-value';
const ORIGINAL_VALUE = 'original-value';
const LABEL = 'test';
const cachedAttrName = getDataAttr(LABEL, ATTR_NAME);
const precedenceAttr = getDataAttr(PRECEDENCE_LABEL, ATTR_NAME);
...
const setup = (isToggled, value) => {
  ...
  if (value) {
    if (isToggled) {
      ...
      // if toggled, add the cached value & the toggled value
      originalEl.setAttribute(precedenceAttr, LABEL);
