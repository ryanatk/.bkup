...
import CheckoutAddressBlock from '../../../../components/pages/my-account/CheckoutAddressBlock';
import CheckoutBillingForm from '../../../../components/pages/my-account/CheckoutBillingForm';
import CheckoutShippingForm from '../../../../components/pages/my-account/CheckoutShippingForm';
import CheckoutSteps from '../../../../components/pages/my-account/CheckoutSteps';
...
import { useSellToCountriesList } from '../../../../queries/Countries';
...
import {
  ...
  reqMyOrderUpdatePaymentMethodShippingAddress,
} from '../../../../stores/myOrder/actions';
import {
  getMyAccountNonceSelector,
  getMyOrderPaymentMethodBillingAddress,
  getMyOrderPaymentMethodShippingAddress,
  getMyOrderUpdatePaymentBillingAddress,
  getMyOrderUpdatePaymentShippingAddress,
...
const Page = ({ authToken }) => {
  ...
  const nonce = useSelector(getMyAccountNonceSelector);
  ...
  } = useMySubscriptionInfo(authToken, nonce);
  const { mutateRenewSubscription } = useRenewSubscription();
  const showAllCountries = checkFeatureFlag('membership-country-show-all');
  const { data: countryData, isLoading: isCountryDataLoading } =
    useSellToCountriesList(showAllCountries);
  ...
  const shippingAddress = useSelector(
    getMyOrderPaymentMethodShippingAddress({ authToken }),
  );
  const savedShippingAddress = useSelector(
    getMyOrderUpdatePaymentShippingAddress({ authToken }),
  );
  ...
  /**
   * Get previously stored shipping address, or shipping address from subscription
   */

  const paymentShippingAddress: CheckoutAddress = savedShippingAddress?.country
    ? savedShippingAddress
    : mapSubscriptionAddressToUiAddress(shippingAddress);
  ...
  const enableCountryDropdown = checkFeatureFlag('membership-country-enabled');
  const myAccountShippingAddressEnabled = checkFeatureFlag(
    'enable-my-account-shipping-address',
  );
   ...
   * ECOM-3775 Ensure that user updates billing address if an error occurred with the payment
   */
  useEffect(() => {
    if (saveResponse?.status) {
      if (saveResponse?.status === OnSaveStatus.Error) {
        setCurrentStep(0);
      } else {
        dispatch({
  ...
  const handleSavePayment = async (response: SavePaymentResponse) => {
    if (
      ...
      metricRequest(RENEW_METRIC_NAME, {
        withShipping: myAccountShippingAddressEnabled,
      });
  ...
  if (isLoading || isCountryDataLoading) return null;
  if (isError)
  ...
  return (
    <MyAccountWrapper authToken={authToken} nonce={nonce}>
      <div className="my-8" data-cy="my-account-update-payment">
        ...
        {saveResponse?.status === OnSaveStatus.Success ? (
          <Alert
            ...
            severity="success"
          >
            {formatMessage({ id: saveResponse.message })}
          </Alert>
        ) : (
          <>
              ...
              steps={[
                {
                  ...
                  RenderEditView: ({ handleContinue, loading }) => (
                    <CheckoutBillingForm
                      loading={loading || isCountryDataLoading}
                      initialValues={paymentBillingAddress}
                      countryData={countryData}
                      disableCountry={
                        !(
                          !billingAddress?.country /* enable the dropdown when no country */ ||
                          /* enable the dropdown for renewal flow */
                          (isRenewFlow && enableCountryDropdown)
                        )
                      }
                      onSubmit={(address) => {
                        // ensure phone is only digits and no symbols
                        const trimmedPhone = address.phone.replace(/\D/g, '');
                ...
                {
                  hidden: !myAccountShippingAddressEnabled,
                  title: t('my_account_payment_shipping_address_title'),
                  RenderEditView: ({ handleContinue, loading }) => (
                    <CheckoutShippingForm
                      loading={loading}
                      countryData={countryData}
                      initialValues={paymentShippingAddress}
                      disableCountry={
                        !(
                          !billingAddress?.country /* enable the dropdown when no country */ ||
                          /* enable the dropdown for renewal flow */
                      ...
                      description="my_account_payment_shipping_address_description"
                      onSubmit={(address) => {
                        // ensure phone is only digits and no symbols
                        const trimmedPhone = address.phone.replace(/\D/g, '');
                  ...
                  RenderCompleteView: () => (
                    <CheckoutAddressBlock address={paymentShippingAddress} />
                  ),
                  onEnter() {
                    metricReporter.increment(
                      'ecom.web.my_account.update_payment.step',
