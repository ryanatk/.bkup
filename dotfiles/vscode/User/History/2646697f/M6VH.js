...
const fillDeliveryAddress = (address = {}, options = {}) => {
  ...
  cy.getCy('field-city').should('exist').clear().type(address.city);

  if (address.country) {
    cy.getCy('field-country')
  ...
  cy.getCy('field-postal').should('exist').clear().type(address.postal);
  cy.getCy('field-phone').should('exist').clear().type(address.phone);
};
