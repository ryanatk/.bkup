...
export const SlideshowImage = ({
  ...
  const isSwappingImages = playAnimation;
  ...
  /**
   * Do not change the "next item" during the animation, because it causes a flicker
   * Instead wait until the animation is complete, so the new "current image" will already be showing.
   */
  useEffect(() => {
    if (!playAnimation) {
  ...
  // const [nextImage, setNextImage] = useState(nextItem);
  ...
  // useEffect(() => {
  //   if (!playAnimation) {
  //     setNextImage(nextItem);
  //   }
  // }, [playAnimation, setNextImage, nextItem]);
  ...
  return (
    ...
    >
      ...
      <Image
        ...
        // {...passthroughImageProps(
        //   nextImage.direction === SLIDESHOW_DIRECTION_FORWARD ? next : prev,
        // )}
        {...nextImageProps}
