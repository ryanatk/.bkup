...
const CartPage = ({ details }): JSX.Element => {
  ...
  // const chargerCount = getCartChargerCount(cart);
  // const isTooManyChargers = chargerCount > maxChargersPerOrder;

  // const cartHasItems = cart?.totalCount > 0;
  // const isTooManyCartItems = isTooManyRings || isTooManyChargers;
  const maxRingsPerOrder = isMultiLineEnabled ? MAX_QUANTITY : 1;
  const maxChargersPerOrder = isChargerSetDisabled ? 0 : MAX_QUANTITY;
  const { ringCount, chargerCount, cartHasItems, isTooManyCartItems } =
    useMemo(() => {
      const ringCount = getCartRingCount(cart);
      const isTooManyRings = ringCount > maxRingsPerOrder;
      ...
      return {
        ...
        isTooManyCartItems: isTooManyRings || isTooManyChargers,
      };
    }, [cart, maxRingsPerOrder, maxChargersPerOrder]);
