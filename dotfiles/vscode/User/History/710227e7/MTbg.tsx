...
const BusinessSolutions = (): ReactElement => {
  ...
  const listWidth = useMemo(
    () =>
      isOnScreen ? imagesList.current?.getBoundingClientRect().width ?? 0 : 0,
    [imagesList, isOnScreen],
  );
  ...
  useEffect(() => {
    const updateWindowY = () => {
      ...
      setState(({ containerX, windowY }) => {
        const scroll = containerX + (window.scrollY - windowY) * scrollSpeed;
        console.log({ scroll, listWidth }, clamp(scroll, 0, listWidth));
        return {
          containerX: clamp(scroll, 0, listWidth),
  ...
  }, [isOnScreen, isMediumScreen, listWidth]);
  ...
  return (
    <Waypoint
      ...
      bottomOffset="10%"
    >
      <div className="relative bg-sand mb-40 py-20 md:py-36">
        <Grid>
          <Title
            className={cx(
