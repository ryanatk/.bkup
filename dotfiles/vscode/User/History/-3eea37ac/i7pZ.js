...
const Checkout = () => {
  ...
  const boothQuestions = useMemo(() => state[STEP.BOOTH_QUESTIONS], [state]);
    ...
    () => getAssessmentList({ eventId, questionType: 2 }),
    {
      ...
      onSuccess: ({ list }) => {
        dispatch({
          ...
          payload: answerQuestions(list, order.assessmentResponseData),
        });
      },
      refetchOnMount: true,
    },
  ...
  const updateQuestions = useCallback(()=>
    dispatch({
      type: TYPE.LOAD_QUESTIONS,
      payload: answerQuestions(boothQuestions, order.assessmentResponseData),
    }),[boothQuestions, order.assessmentResponseData]
  )
