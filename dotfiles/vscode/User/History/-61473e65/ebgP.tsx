  ...
  if (prefersReducedMotion || !videoEnabled) {
    return (
      <Image
        ...
        src={src(imageData.src, 'jpg', matchLargeScreen ? 1920 : 1024)}
...
const ContentSection = ({
  ...
  const { className, title, body, color, isLightText } = useHeroVariant();
...
const Hero = ({ headerHeight }: HeroProps): JSX.Element => {
  ...
  return (
    <Wrapper
      ...
      darkMode={!isLightText}
    >
      <Waypoint
        onEnter={() => {
          if (!visible) setVisible(true);
          setInverse(isLightText);
        }}
        onLeave={() => {
          setInverse(false);
        }}
