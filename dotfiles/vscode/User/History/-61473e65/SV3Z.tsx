...
const Background = ({
  ...
  const { background } = useHeroVariant();
  ...
  const imageData = useMemo((): {
    ...
    return {
      alt: background.alt,
      src: matchLargeScreen ? background.desktop : background.mobile,
    };
  }, [matchLargeScreen, background]);
...
const ContentSection = ({
  ...
  const { className, title, body, color } = useHeroVariant();
  ...
  return (
    <Typography Element="div" color={color} className={cx(styles.content, styles[className])}>
      <Typography
        Element="h2"
        color="inherit"
        weight="light"
      ...
      <HeroButton
        variant={color === 'white' ? 'secondary' : 'primary'}
        visible={visible}
      />
...
const Hero = ({ headerHeight }: HeroProps): JSX.Element => {
  ...
  const { color: headerColor } = useHeroVariant();
