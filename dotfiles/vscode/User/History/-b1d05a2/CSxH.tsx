  ...
  const limit = useMemo(() => {
    ...
    if (hasRings && hasChargers) {
      if (maxRings === 1 && maxChargers === 1) {
        return t('cart_one_ring_one_charger_per_order');
      } else if (maxRings === 1) {
        return t('cart_one_ring_max_chargers_per_order', { maxChargers });
      } else if (maxChargers === 1) {
        return t('cart_max_rings_one_charger_per_order', { maxRings });
      } else {
        return t('cart_max_rings_max_chargers_per_order', {
          maxRings,
          maxChargers,
        });
      }
    ...
    if (hasRings) {
      if (maxRings === 1) {
        return t('cart_one_item_per_order', { maxRings });
      } else {
