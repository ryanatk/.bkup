...
export class ProductPageHelper extends BasePageHelper {
  ...
  verifyPrice(price, currency) {
    cy.getCy('product-detail-discount-price').contains(price);
    if (currency) {
      cy.getCy('product-detail-discount-price').contains(currency);
    }
  }

  verifyPriceForLocale(locale) {
    if (locale === 'eu') {
      this.verifyPrice(314, 'EUR');
