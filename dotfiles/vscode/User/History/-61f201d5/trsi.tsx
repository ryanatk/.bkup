...
import usePartnerUtm from '../../../hooks/usePartnerUtm';
...
const StyleBasePrice = ({ style }: { style: RingStyle }): JSX.Element => {
  const { formatNumber } = useIntl();
  const basePriceForRings = useRingsBasePrice();
  const currency = useSelector((state: State) => state.app.currency);
  const { amount } = basePriceForRings[style];
  ...
  return (
    <PriceLabel>
      <FormattedMessage id="pdp_from" values={{ amount: formattedAmount }} />
    </PriceLabel>
  );
};
...
const ProductSelection = ({
  ...
  const { isMatch: isPartnerTherabody } = usePartnerUtm({
    medium: 'partner',
    source: 'therabody',
    campaign: 'retail_store',
  });
