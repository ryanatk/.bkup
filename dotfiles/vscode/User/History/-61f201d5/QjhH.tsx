...
import useRingsBasePrice from '../shared/hooks/useRingsBasePrice';
...
const ProductSelection = ({
  ...
  useEffect(() => {
    // ensure product is loading before updating size
    if (!product?.handle) {
      return;
    }

    if (sizeOnly) {
      // ensure size is chosen for accessories
      if (size === RING_SIZE_NOT_SELECTED) {
        setSize(null);
      }
    } else {
      // default to "no size" option, when allowed
      if (!size) {
        setSize(RING_SIZE_NOT_SELECTED);
      }
    }
  }, [sizeOnly, size, setSize, product]);
    ...
    <>
          ...
          <div css={tw`my-8`}>
            ...
            >
              {RINGS_BY_FINISH.map((ring, i) => (
                <SelectButton
                  data-cy="pdp-finish-option"
                  data-value={ring.finish.slug}
                  key={`ring-${i}`}
                  css={tw`
                    flex
                    flex-col
      ...
      >
        ...
        <ButtonWrapper
          ...
          role="radiogroup"
          tabIndex={-1}
          aria-labelledby="sizing-group-label"
        ...
        >
          {!sizeOnly && (
            <SelectButton
              ...
              data-cy="pdp-size-option"
              data-size={RING_SIZE_NOT_SELECTED}
            />
          )}
