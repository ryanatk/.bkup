...
describe('cartCount > getCartRingCount', () => {
...
describe('cartCount > getCartChargerCount', () => {
  it('counts number of chargers correctly in a cart', () => {
    expect(
      getCartChargerCount(
        MOCK_CART_WITH_RINGS_AND_CHARGER_SETS as unknown as CartState,
      ),
  ...
  it('counts number of chargers correctly in a cart when there are no chargers', () => {
    expect(
      getCartChargerCount(
        MOCK_CART_WITH_NO_RINGS_OR_CHARGER_SETS as unknown as CartState,
      ),
  ...
  it('counts number of chargers correctly in a cart when empty', () => {
    expect(
      getCartChargerCount(MOCK_CART_EMPTY as unknown as CartState),
    ).toEqual(0);
  });
});
