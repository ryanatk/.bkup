...
const BoothSetup = () => {
  ...
  const steps = useMemo(
    () =>
      list?.map((item, index) => {
        const step = getStep({ item, list, index, history });
        console.log({ item, step });

        return merge({}, item, step, {
          isReady,
          submit: (response) =>
            submit({
              onSuccess: step.next,
              assessmentResponseData: {
                assessmentQ_ID: step.assessmentQ_ID,
                id: step.id,
                response,
              },
              ooId: ooSummaryId,
              isOptional: !step.question.required,
            }),
        });
      }),
    [list, isReady, submit, history, ooSummaryId],
  );
