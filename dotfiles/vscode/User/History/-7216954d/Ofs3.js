...
import { calculateCartTotals, date, DATE_FORMAT, track } from 'common/utils';
import {
  SubmitButton,
  TermsOfUseLink,
  PrivacyPolicyLink,
  ...
  const { mutate: submit, isLoading: isSubmitting } = useMutation(
    processCartPayment,
    {
      onSuccess: () => {
        track('purchase', {
          currency: 'USD',
          transaction_id: ooSummaryId,
          value: recalculatedTotals.total,
          items: [],
        });
        history.push(ROUTE.CONFIRMATION + '/' + ooSummaryId);
      },
