...
export default function CartService(state: State) {
  ...
  async function addItemsToCart({
    ...
    console.log('FUCK', { updatingExistingLineItem }, cart.lineItems);
    try {
      ...
      const isExistingParent =
        updatingExistingLineItem ||
        // only "chargers" that are already in the cart are "existing parents"
        (lineItem && (await fetchFeatureFlag('enable-multi-line-items')));

      console.log({ lineItem, isExistingParent });
      ...
      if (isExistingParent) {
        newCart.lineItems.splice(lineItemIndex, 1, {
          ...newProduct,
          // quantity: newProduct.quantity + lineItem.quantity,
        });
      } else {
