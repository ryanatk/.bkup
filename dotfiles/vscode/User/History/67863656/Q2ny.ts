...
import { appFlagsSelector } from '../stores/app/selectors';
...
export default function CartService(state: State) {
  ...
  async function addItemsToCart({
    ...
    try {
      ...
      const isChargerQuantityEnabled = appFlagsSelector(
        'enable-charger-quantity',
      );
      const isExistingParent =
        lineItem && isCharger(lineItem) && isChargerQuantityEnabled;
