...
export default function CartService(state: State) {
  ...
  async function addItemsToCart({
    ...
    try {
      const multiRingFlag = await fetchFeatureFlag('enable-multi-line-items');
      ...
      const lineItem = cart.lineItems.find(({ id }) => id === variantId);
      const isExistingParent = lineItem && isCharger(lineItem);
      ...
      const lineItems = isExistingParent
        ? cart.lineItems.filter(({ id }) => id !== lineItem.id)
        : cart.lineItems;
      console.log({ lineItem, isExistingParent, newProduct, lineItems });
      newCart.lineItems = [...lineItems, newProduct];
