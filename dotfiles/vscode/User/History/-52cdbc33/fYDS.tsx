...
const getThumbails = (
  ...
  const isHidden = (src: string) =>
    HIDDEN_THUMBS.some((str) => src.includes(str));

  return variantId === VariantId.One
    ? images
        .filter(({ originalSrc }) => !isHidden(originalSrc))
        .sort(({ originalSrc }) => (isVideo(originalSrc) ? -1 : 0))
    : images;
};
...
const ProductHorizonSlideshow = (): JSX.Element => {
  ...
  //   ? getThumbails(product.images, variantId)
  //   : [];
  const images: PDPData_content_productByHandle_images[] = ready
    ? getThumbails(product.images, VariantId.One)
    : [];
