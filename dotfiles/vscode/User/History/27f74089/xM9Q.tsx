...
const Paypal = ({
  ...
  useEffect(() => {
    if (!paymentGateway || !paypalMerchantId) {
      console.log('error', { paypalLoadingError });
      if (!paypalLoadingError) {
        setPaypalLoadingError(true);
      }
    }
  }, [paymentGateway, paypalMerchantId, paypalLoadingError]); // only once

  useEffect(() => {
    console.log({ isButtonAdded });
  ...
  useEffect(() => {
    if (!loading && !paypalLoadingError && query.token && query.gateway) {
  ...
  }, [loading, paypalLoadingError, query, checkoutCallback]);
  // }, []);
