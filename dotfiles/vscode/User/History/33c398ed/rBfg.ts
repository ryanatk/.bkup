...
import sellToContries from '../static/data/countries.json';
...
export default class Utils {
  ...
  static getSellToCountriesList(): {
    ...
    return Object.keys(sellToContries)
      .filter((c) => sellToContries[c].sellTo !== false)
      .map((c) => {
        return {
          label: `${sellToContries[c].name}`,
          value: sellToContries[c].countryCode,
          disableForBilboLaunch: sellToContries[c].disableForBilboLaunch,
        };
      });
  }
  ...
  static getCountryCallingCodes(): {
    ...
    return Object.keys(sellToContries).map((c) => {
      const obj = {
        key: sellToContries[c].countryCode,
        value: sellToContries[c].countryCode,
        flag: sellToContries[c].countryCode.toLowerCase(),
        text: sellToContries[c].calling_code,
      };
      if (sellToContries[c].sellTo) return obj;
    });
  }
  ...
  static getCountryCallingCode(code: string): string | false {
    if (sellToContries[code] && sellToContries[code].calling_code) {
      return sellToContries[code].calling_code;
    }
    return false;
  }
  ...
  static getSellToCountry(countryCode: string): SellToCountry | false {
    ...
    return sellToContries[countryCode];
  }
  ...
  static getCountryFullName(
    ...
    if (!countryCode) return false;
    return sellToContries[countryCode].name || false;
  }
