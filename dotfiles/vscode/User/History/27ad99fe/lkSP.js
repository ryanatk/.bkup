...
export const CartProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    console.log(
      {
        isTouched: cart.isTouched,
        queue: !Object.keys(cart.queue).length,
        unsettled: !cart.unsettled.length,
      },
      cart.queue,
      cart.unsettled,
    );
  ...
  useEffect(() => {
    ...
    // reshape to an array, and ignore anything with unsettled requests
    const queue = Object.entries(cart.queue).reduce((list, [id, item]) => {
      const isUnsettled = cart.unsettled.includes(Number(id));
      console.log({ isUnsettled }, cart.unsettled, Number(id));
      return isUnsettled ? list : [...list, item];
    }, []);

    queue.forEach(({ quantity, ...queueItem }) => {
      const item = merge({}, queueItem, findItem(cart.items, queueItem));
