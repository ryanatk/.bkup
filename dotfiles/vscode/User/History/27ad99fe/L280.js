...
export const CartProvider = ({ children }) => {
  ...
  const { mutate: updateItem } = useMutation(
    ({ item, quantity }) => {
      ...
      const isInCart = Boolean(item?.cartId);
      console.log({ isInCart, item, quantity });
  ...
  useEffect(() => {
    ...
    const queue = Object.entries(cart.queue).reduce((list, [id, item]) => {
      console.log({ id }, '****** unsettled', cart.unsettled);

      return cart.unsettled.includes(id) ? list : [...list, item];
    }, []);

    console.log({ queue }, 'unsettled', cart.unsettled);
    ...
    queue.forEach((item) => {
      ...
      updateItem({ item: cartItem, quantity: item.quantity });
    });
