...
// helper function to get bounded quantity of an item, for update
const getBoundedQuantity = ({ max, min }, quantity) => {
  if (quantity < min) {
    return min;
  }
  if (max && quantity > max) {
...
export const CartProvider = ({ children }) => {
  ...
  const { mutate: updateItem, isLoading: isUpdating } = useMutation(
    ({ item, quantity: unboundedQuantity }) => {
      // console.log('!updateItem', { item, quantity });

      const quantity = getBoundedQuantity(item, unboundedQuantity);

      if (quantity !== unboundedQuantity) {
        // show toast
      }
