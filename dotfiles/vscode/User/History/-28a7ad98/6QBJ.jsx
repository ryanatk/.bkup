...
const setup = (options = {}) => {
  ...
  const storage = hasStorage ? { utm_medium: STORAGE_VALUE } : undefined;
  const matchParams = match?.length ? { utm_medium: match } : {};

  loadState.mockReturnValue(storage);
...
describe('usePartnerUtm', () => {
  ...
  test('clear method removes item from storage', () => {
    const { result } = setup();
    act(() => result.clear());
    expect(clearState).toBeCalledWith(STORAGE_KEY);
  ...
  describe('with no query params & a partner in storage', () => {
    const OPTIONS = { hasStorage: true };

    test('value is loaded from storage', () => {
      setup(OPTIONS);
      expect(loadState).toBeCalledWith(STORAGE_KEY);
    });

    test('utm is returned', () => {
      const { result, storage } = setup(OPTIONS);
      expect(result.utm).toEqual(expect.objectContaining(storage));
    });
