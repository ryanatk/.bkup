...
describe('getCountriesList', () => {
  it('contains some specific example countries', () => {
    const sellToCountries = Utils.getCountriesList();
    const byCode = {};
    ...
    expect(sellToCountries.length).toBeGreaterThan(63);
...
describe('getSellToCountriesList', () => {
  it('contains some specific example countries', () => {
    const countries = Utils.getSellToCountriesList();
    const byCode = {};
    ...
    expect(byCode['FR']).toEqual({ label: 'France', value: 'FR' });
    expect(byCode['US']).toEqual({ label: 'United States', value: 'US' });
    expect(countries.length).toBeGreaterThan(43);
