...
const Title = styled(Typography)(({ isSantaComplete }) => [
  ...
  isSantaComplete
    ...
    leading-tight
    text-4xl
    md:(text-7xl tracking-[-2px] leading-6)
    xl:(text-[5.25rem])
    `,
]);
...
const Body = styled(Typography)(({ isSantaComplete }) => [
  tw`
    ...
    lg:(flex-shrink-0 w-full)
  `,
  isSantaComplete && tw`order-1 lg:order-2`,
]);
...
const Background = ({
  ...
  const imageData = useMemo((): {
    ...
    if (isSantaComplete) {
      return {
        src: `simple-home/${srcPrefix}-hero-img-santa-complete`,
...
const HeroButton = ({
  ...
  return (
    <a.div className="mt-6 lg:mr-6 order-3" style={ctaSpring}>
      <Button
        variant={variant}
        shade="dark"
        size="large"
    ...
    <Content>
      ...
      <Body color="white" data-cy="hero-body" isSantaComplete={isSantaComplete}>
        <a.span className="inline-block" style={eyebrowSpring}>
          {isSantaComplete
            ? t('home_hero_santa_complete_eyebrow')
