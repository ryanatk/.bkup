...
const HorizonPDP = ({
  ...
  const seoParams = useMemo(() => {
    const { title: productTitle, description: productDescription } =
      product?.seoParams ?? {};

    const title = formatMessage({ id: product?.seoParams?.title });
    const description = formatMessage({ id: product?.seoParams?.description });
