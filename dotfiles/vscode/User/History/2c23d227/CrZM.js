...
const AddToCart = ({ item }) => {
  ...
  const handleUpdateQuantity = useCallback(
    (qtyAdd) => {
      console.log('!handleUpdateQuantity', { qtyAdd });

      const remainder = qtyAdd % item.updateIncrement;

      if (remainder) {
        setState({
          toast: `must add multiple of ${item.updateIncrement}`,
  ...
  const handleSubmit = useCallback(
    (qtyAdd) => {
      ...
      if (qtyAdd + item.quantity < item.addIncrement) {
        submit(item.addIncrement, `must add at least ${item.addIncrement}`);
      } else if (qtyAdd + item.quantity > item.max) {
        submit(item.max, `max is ${item.max}`);
      } else {
