...
const AddToCart = ({ item }) => {
  ...
  const handleSubmit = useCallback(
    (qtyAdd) => {
      ...
      if (qtyAdd + item.quantity < item.addIncrement) {
        submit(
          item.addIncrement,
          `You must have a minimum of ${item.addIncrement} ${item.name} per order. We've added them to your cart.`,
        );
      } else if (qtyAdd + item.quantity > item.max) {
        submit(
          item.max,
          `You can only have a maximum of ${item.max} ${item.name} per order. We've added them to your cart.`,
        );
      } else {
  ...
  const min = useMemo(() => item.updateIncrement, [item]);
