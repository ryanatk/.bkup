...
const Slideshow = ({
  ...
  // using a combined state to avoid race conditions when updating multiple values at once
  // (updating separately was causing a "flicker" when `nextItem` updates before `currentIndex`)
  ...
  const { currentIndex, refreshImages, nextItem } = useMemo(
    () => state,
    [state],
  );

  // helper fn (sugar) to ease the burden of combined state updates
  ...
  return (
    <SlideshowContext.Provider
      value={{
        ...
        nextItem,
        refreshImages,
        setCurrentIndex: (currentIndex) => updateState({ currentIndex }),
        imageLoading,
        slidesTransitioning,
