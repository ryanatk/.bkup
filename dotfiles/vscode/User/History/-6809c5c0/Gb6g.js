...
it('VALID recognizes an invalid phone number that is too short', async () => {
  const result = await setup('1234567');
  expect(result).toEqual(false);
});

it('VALID recognizes an invalid phone number that contains a letter', async () => {
  const result = await setup('123456789a');
  expect(result).toEqual(false);
});
