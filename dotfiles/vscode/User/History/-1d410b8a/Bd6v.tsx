...
const Paypal = ({
  ...
  const [paypalLaodingError, setPaypalLaodingError] = useState(false);
  const { query } = router;

  useEffect(() => {
    if (!paymentGateway || !paypalMerchantId) {
      setPaypalLaodingError(true);
  ...
  const handlePaypalFlow = async () => {
    ...
    const { status, response } = await setExpressCheckout({
      ...
      returnUrl: `${window.location}?gateway=${paymentGateway}`, // TODO: need to compose the url properly
      cancelUrl: `${window.location}?status=canceled`,
